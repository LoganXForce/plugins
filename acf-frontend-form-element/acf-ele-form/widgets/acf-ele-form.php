...
class ACF_Elementor_Form_Widget extends Widget_Base
{
            ...
            if ( !is_user_logged_in() ) {
                ...
                if ( $settings['dynamic'] ) {
                    ...
                    if ( '[author]' == $settings['dynamic'] ) {
                        ...
                        if ( isset( $settings['post_to_edit'] ) ) {
                            
                            if ( $settings['post_to_edit'] == 'url_query' ) {
                                if ( isset( $_GET[$settings['url_query_post']] ) && $settings['url_query_post'] ) {
                                    $current_user = get_post_field( 'post_author', $_GET[$settings['url_query_post']] );
                                }
                            } else {
    ...
    protected function render()
    {
        ...
        if ( 'new_comment' == $settings['main_action'] ) {
            $post_id = 'new_comment';
            $comment_action = $module->get_main_actions( 'comment' );
            $defaults = $comment_action->render_form( $settings, $wg_id );
        ...
        if ( 'new_comment' == $settings['main_action'] ) {
            $hidden_fields .= '<input type="hidden" value="' . $comment_parent_post . '" name="acfef_parent_post"/>';
        }
        ...
        if ( strpos( $settings['main_action'], '_post' ) !== false ) {
            ...
            if ( $settings['default_featured_image']['id'] ) {
                $featured_image = $settings['default_featured_image']['id'];
            }
