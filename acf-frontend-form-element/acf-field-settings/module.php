...
class ACFS_Module extends Module
{
    ...
    public function acfef_custom_tf_field( $field )
    {
        acf_render_field_setting( $field, array(
    ...
    public function acfef_save_value( $post_id )
    {
        ...
        if ( $edit_post ) {
            ...
            if ( $form && !empty($form['featured_image']) ) {
    ...
    public function acfef_render_text_field( $field )
    {
        ...
        if ( isset( $field['custom_sku'] ) && $field['custom_sku'] == 1 ) {
            echo  '<input type="hidden" name="custom_sku" value="' . $field['key'] . '"/>' ;
        }
    }
    
    public function acfef_render_tf_field( $field )
    {
        if ( isset( $field['custom_sold_ind'] ) && $field['custom_sold_ind'] == 1 ) {
            echo  '<input type="hidden" name="custom_sold_ind" value="' . $field['key'] . '"/>' ;
        }
    }
    ...
    public function acfef_render_select_field( $field )
    {
        ...
        if ( isset( $field['custom_stock_status'] ) && $field['custom_stock_status'] == 1 ) {
            echo  '<input type="hidden" name="custom_stock_status" value="' . $field['key'] . '"/>' ;
        }
    }
    ...
    public function acfef_pre_save_post( $post_id )
    {
        ...
        if ( $edit_post ) {
            ...
            if ( isset( $_POST['custom_price'] ) ) {
                ...
                unset( $_POST['acf'][$_POST['custom_price']] );
            ...
            if ( isset( $_POST['custom_sale_price'] ) ) {
                ...
                unset( $_POST['acf'][$_POST['custom_sale_price']] );
            }
            ...
            if ( isset( $_POST['custom_sku'] ) ) {
                $sku = $_POST['acf'][$_POST['custom_sku']];
                unset( $_POST['acf'][$_POST['custom_sku']] );
                update_post_meta( $post_id, '_sku', $sku );
            }
            
            
            if ( isset( $_POST['custom_stock_status'] ) ) {
                $stock_status = $_POST['acf'][$_POST['custom_stock_status']];
                unset( $_POST['acf'][$_POST['custom_stock_status']] );
                update_post_meta( $post_id, '_stock_status', $stock_status );
            }
            
            
            if ( isset( $_POST['custom_sold_ind'] ) ) {
                
                if ( $_POST['acf'][$_POST['custom_sold_ind']] ) {
                    update_post_meta( $post_id, '_sold_individually', 'yes' );
                } else {
                    update_post_meta( $post_id, '_sold_individually', 'no' );
                }
                
                unset( $_POST['acf'][$_POST['custom_sold_ind']] );
            }
    ...
    function acfef_fields_enqueue_scripts()
    {
        wp_enqueue_script(
            'acfef-fields',
            ACFEF_URL . 'acf-field-settings/assets/js/fields.js',
            array( 'jquery' ),
            '4.0'
        );
    }
    ...
    public function __construct()
    {
        ...
        if ( class_exists( 'woocommerce' ) ) {
            ...
            add_action( 'acf/render_field_settings/type=true_false', [ $this, 'acfef_custom_tf_field' ] );
        ...
        add_action( 'acf/render_field/type=true_false', [ $this, 'acfef_render_tf_field' ] );
